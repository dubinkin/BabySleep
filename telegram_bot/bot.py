...
async def cmd_start(message: types.Message):
    await message.reply('Hello! I can tell you how sleepy your baby is based on the sleeping '
                        'data you collected via BabyTracker app. I can also learn your baby\'s '
                        'sleep patterns and suggest a natural sleeping schedule over the next 24 hours for them.')
    await bot.send_message(chat_id=message.from_user.id, text= 'Please forward me CSV data from the app to get my estimate.')
...
async def download_doc(message: types.Message):
    ...
    await bot.send_message(chat_id = message.from_user.id, text = 'Sleepy score: {p:1d} out of 3'.format(p = score+1),
                           parse_mode='Markdown')
    #await bot.send_message(chat_id=message.from_user.id, text='Raw score: {p:.2f} out of 10'.format(p = rawscore),
      #                     parse_mode='Markdown')
    await bot.send_message(chat_id = message.from_user.id, text = sleepdict[score])

    keyboard = types.ReplyKeyboardMarkup(resize_keyboard=True)
    buttons = ["Yes", "No"]
    keyboard.add(*buttons)
    await message.answer('Would you like me to generate a suggested sleeping schedule for the next 24 hours?', reply_markup=keyboard)

@dp.message_handler(Text(equals='Yes'))
async def yes_generate(message: types.Message):
    await message.reply('Give me about 5 to 10 minutes...', reply_markup=types.ReplyKeyboardRemove())
    ...
    # compute the
    ans, pred1, pred2, pred3 = ensemble.forecast(sleep, time0, DATA_PATH, MODEL_PATH, forecast_period, weights, conv,
                                                 conv)
    ...
    await bot.send_photo(chat_id = message.from_user.id, photo=open('../predictions/forecast.png', 'rb'))
...
async def dont_generate(message: types.Message):
    await message.reply("All right! See you soon!", reply_markup=types.ReplyKeyboardRemove())
    # process the data
    sleep, time0 = load_data.get_sleep('12/06/2021', '../data/')
    MODEL_PATH = '../src/models/'
